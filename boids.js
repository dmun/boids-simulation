...
class Scene {
  ...
  getOrder() {
    let o = [0, 0];
    for (let p of this.swarm) {
      o = p.addVectors(o, p.dir);
    }
    return this.magnitude(this.multiplyVector(o, 1 / this.swarm.length));
  }

  getNNDistances() {
    let d = [];
    for (let x of this.swarm) {
      d.push(this.getNN(x));
    }
    return d;
  }
  ...
  getNN(x) {
    ...
    for (let p of this.swarm) {
      ...
      d.push(this.dist(this.wrap(p.pos, x.pos), x.pos));
    }
    return Math.min(...d);
  }
  ...
  neighbours(x, distanceThreshold) {
    ...
    for (let p of this.swarm) {
      ...
      if (this.dist(this.wrap(p.pos, x.pos), x.pos) <= distanceThreshold) {
        r.push(p);
      }
    }
    return r;
...
class Particle {
  ...
  alignmentVector(neighborRadius) {
    let v = [0, 0];
    for (let p of this.S.neighbours(this, neighborRadius)) {
      v = this.addVectors(v, p.dir);
    }
  ...
  cohesionVector(neighborRadius) {
    let v = [0, 0];
    for (let p of this.S.neighbours(this, neighborRadius)) {
      v = this.addVectors(v, this.subtractVectors(this.S.wrap(p.pos, this.pos), this.pos));
    }
    return this.normalizeVector(v);
  }
  ...
  separationVector(neighborRadius) {
    let v = [0, 0];
    for (let p of this.S.neighbours(this, neighborRadius)) {
      v = this.addVectors(v, this.subtractVectors(this.pos, this.S.wrap(p.pos, this.pos)));
    }
    return this.normalizeVector(v);
  }

  magnitude() {
    return Math.sqrt(Math.pow(this.dir[0], 2) + Math.pow(this.dir[1], 2));
  }
...
function initialize() {
  ...
  //Plotly.newPlot("plotAngles", data, layout);
}
